...
- Google PubSub Topic using the `terraform-google-module-template` module (source: `github.com/subhamay-bhattacharyya-tf/terraform-google-module-template`)
- Follow the GCP provider reference and core authoring patterns from the `tf-mod-main` skill

**locals.tf:**

A map type variable must be created from the input variable and the pubsub topic name must be in the following format:
...
| `pubsub_subscription_config` | `object` | Yes | See attribute table below |
...
- Outputs for all standard Google Pub/Sub topic attributes:
  - `topic_id`
  - `topic_name`
...
- `test/pubsub_topic_basic_test.go`: This Terratest tests the basic pubsub topic configuration.
